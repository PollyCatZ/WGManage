        ...
        if regex_match("^(.{1,}).(conf)$", i):
            ...
            create_table = f"""
                CREATE TABLE IF NOT EXISTS {i} (
                    ...
                    keepalive INT NULL, remote_endpoint VARCHAR NULL, preshared_key VARCHAR NULL, active SMALLINT NULL,
                    PRIMARY KEY (id)
                )
            """
...
def add_peer_bulk(config_name):
    ...
    for i in range(amount):
        ...
        update = ["UPDATE ", config_name, " SET name = '", keys[i]['name'],
                  ...
                  "', endpoint_allowed_ip = '", endpoint_allowed_ip, "', active = '1' WHERE id = '", keys[i]['publicKey'], "'"]
...
def add_peer(config_name):
    ...
    try:
        ...
        status = subprocess.check_output("wg-quick save " + config_name, shell=True, stderr=subprocess.STDOUT)
        get_all_peers_data(config_name)
        sql = "UPDATE " + config_name + " SET name = ?, private_key = ?, DNS = ?, endpoint_allowed_ip = ?, active = ? WHERE id = ?"
        g.cur.execute(sql, (data['name'], data['private_key'], data['DNS'], endpoint_allowed_ip, 1, public_key))
        return "true"
    except subprocess.CalledProcessError as exc:
        return exc.output.strip()
